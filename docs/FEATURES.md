...
via argv-only `git blame`/`git show` (no shell). Mode-aware
(`on_hold.modes`), per-window throttled, and cleared on the next cursor move
via a generation counter that invalidates stale delayed runs after a mode
change. Skips buffers outside a git repo, untracked files (when
`only_tracked`), and files with unsaved changes (when
`require_clean_buffer`). Sets `vim.o.updatetime = 100` when enabled to match
the responsiveness the fallback preview needs.
